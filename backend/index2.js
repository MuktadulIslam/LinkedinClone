...
async function getImageUrl(bucketName, imageName) {
...
app.post('/post', upload.single('image'), async (req, res) => {
    ...
    try {
        if (req.file) {
            ...
            minioClient.putObject(bucketName, imageName, imageBuffer, metaData, async (err, etag) => {
                if (err) {
                    console.error('Error uploading image:', err);
                } else {
                    // console.log('Image uploaded successfully with etag:', etag);
                    // const imageUrl = minioClient.presignedGetObject(bucketName, imageName);
                    // console.log(imageUrl);
                    const imageUrl = await getImageUrl(bucketName, imageName);
                    console.log(imageUrl);
                }
            });
        }
        ...
        getConnection().query(
            "INSERT INTO posts (name, email, date, article_text, image, postID) VALUES (?, ?, ?, ?, ?, ?)",
            [name, email, date, article, image, postID],
            (err, result) => {
                if (result) {
                    try {
                        ...
                        getConnection().query("SELECT email FROM users", (err, result) => {
                            result.forEach(useremail => {
                                if (useremail.email !== email) {
                                    getConnection().query(
                                        "INSERT INTO notification (email, postID) VALUES (?, ?)",
                                        [useremail.email, postID],
                                        (err, result) => {
                                            if (err) console.log(err);
                                        }
                                    );
